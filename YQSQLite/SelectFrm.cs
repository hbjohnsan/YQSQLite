...
namespace YQSQLite
{
    public partial class SelectFrm : DockContent
    {
        ...
        private void btnAdd_Click(object sender, EventArgs e)
        {
            ...
            if (listView1.SelectedItems.Count > 0)
            {
                for (int i = 0; i < listView1.SelectedItems.Count; i++)
                {
                    ...
                    SQLiteDS.RssItemRow rssrow = mf.DS.RssItem.FindByTitle(rssitem.Title);
                    rssrow.IsRead = "待处理";
                    rssrow.Content = htmlEditor1.HTML;
                    mf.NewsAdd(rssitem.Title);
                    // listBox1.Items.Remove(rssitem);
                    listView1.SelectedItems[0].Remove();
                    mf.rssTap.Update(mf.DS.RssItem.FindByTitle(rssitem.Title));
                    htmlEditor1.HTML = "";
                }
            }
        }
        ...
        private void btnSelectAll_Click(object sender, EventArgs e)
        {
            ...
            {
                RssItem ri = lv.Tag as RssItem;
                SQLiteDS.RssItemRow rssrow = mf.DS.RssItem.FindByTitle(ri.Title);
                rssrow.IsRead = "已读";
            }
            mf.rssTap.Update(mf.DS.RssItem);
            listView1.Items.Clear();
